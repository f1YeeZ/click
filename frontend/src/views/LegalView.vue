...
const documents = {
  privacy: {
    kicker: '隐私与数据', title: '隐私政策', intro: `本政策说明${operatorName}如何处理你在使用 GearDB 时提供的数据。`,
    sections: [
      ['我们收集的信息', ['账号信息：邮箱地址、加密后的密码及协议同意时间。', '个人资料：手长、手型分类和习惯握姿。', '支撑记录：支撑位置、关联鼠标及提交时间。', '运行数据：必要的访问日志、IP 地址、错误信息和安全事件记录。']],
  ...
  terms: {
    kicker: '服务条款', title: '用户协议', intro: `本协议是你与${operatorName}之间关于使用 GearDB 的约定。`,
    sections: [
      ...
      ['禁止行为', ['不得批量注册、伪造支撑记录、操纵推荐、攻击接口、绕过访问控制、上传恶意文件、抓取非公开数据或实施其他违法及损害服务稳定性的行为。']],
  ...
  rules: {
    kicker: '记录规范', title: '提交规则', intro: '这些规则用于保证支撑位置记录可比较、可聚合，并减少误导。',
    sections: [
      ['真实体验', ['仅提交你实际使用过的鼠标。支撑位置应来自自然握持体验，不得因返利、赠品、竞争关系或组织行为伪造记录。']],
      ['结构化记录', ['不同握姿的支撑位置分别记录，并按照个人资料中的手长与习惯握姿进行聚合。']],
      ['支撑位置', ['请选择鼠标在自然握持状态下真实托住手掌的部位，而不是希望鼠标支撑但实际没有接触的位置。推荐系统只把同一份评价覆盖的全部必要位置视为完全匹配。']],
      ['样本与展示', ['低于 5 份的样本会标记为“样本较少”。热力图和推荐会随有效记录变化。']],
      ['修改、删除与治理', ['你可以删除自己的支撑记录。被删除或管理员停用的记录不再参与公开聚合；涉嫌自动化提交或明显异常的数据可被停用。']],
...
<template>
  <main class="legal-page section-shell">
    ...
    <div class="legal-layout">
      <aside><strong>{{ operatorName }}</strong><span>生效日期：{{ updatedAt }}</span><RouterLink to="/privacy">隐私政策</RouterLink><RouterLink to="/terms">用户协议</RouterLink><RouterLink to="/review-rules">提交规则</RouterLink></aside>
