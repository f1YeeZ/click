...
| --- | --- | --- |
| 鼠标库、详情、筛选、2～4 款对比、固定评价、评分排行、精确/相近推荐及解释、产品图片、邮箱登录与忘记密码、CSV 导入、管理后台 | 鼠标轮廓/轮廓叠加/3D 鼠标模型、自由文本、商品价格、其他外设品类、逐条公开评价 | 评价举报与持有验证、第三方登录、收藏、纠错反馈和订阅 |
...
```json
{
  ...
  "dimensionDistributions": {
    "comfort": { "1": 0, "2": 0, "3": 0, "4": 1, "5": 1, "6": 2, "7": 3, "8": 6, "9": 8, "10": 6 }
  },
...
`dimensionDistributions` 对五个整数评分维度分别统计 1～10 分数量，不对带小数的综合评分做主观分桶。标签被管理员停用后不可再选择，历史关联保留用于审计，但不再进入公开汇总。

基础评价由按键、滚轮、做工和涂层四项评分组成；握持舒适度按握姿单独提交。单条记录存在舒适度时按五项等权计算，否则按四项基础评分计算，结果四舍五入保留 1 位并存为 `DECIMAL(3,1)`。公开汇总仅对有样本的维度求均值，并在 API 响应阶段保留 1 位小数。
...
| 评分维度 | 基础四项（按键、滚轮、做工、涂层）与独立握姿舒适度 | 基础汇总不受握姿筛选影响，握姿汇总随条件变化 |
| 综合分 | 基础综合分与握姿综合分分别展示 | 避免把不同样本口径混成一个难解释的分数 |
