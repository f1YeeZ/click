...
> 当前实现已移除 1～10 分评分、评分排序、分数汇总和排行榜。本文后续仍出现的评分字段仅用于说明旧版数据模型，不再构成当前产品功能或 API 契约。
...
首页桌面首屏使用 `calc(100dvh - 68px)` 填满页头下方的可见区域，不得用固定像素上限截断首屏高度；这样 1080p 与 2K 等常见 16:9 视口都会在首屏下方保留一致的动画滚动距离。移动端继续使用内容自然高度。“近期新品”模块使用 `top+=100 bottom` 的 ScrollTrigger 起点：模块顶部以下 100px 到达视口底部后才开始入场，避免高分辨率视口在页面加载时直接越过触发线。模块主体使用约 950ms 的舒缓回弹，首组卡片使用约 650ms 的入场并以 75ms 间隔错峰出现；入场只播放一次，页面离开时必须清理 ScrollTrigger。模块插入 DOM 时必须由 `[data-reveal-state='pending']` CSS 同步隐藏，不能等到 GSAP 下一帧写入内联样式，以免标题在首页首帧闪现。该入场动画与 `trending-loop` 自动横向轮播相互独立，完成入场后轮播继续运行，悬停或键盘焦点进入时暂停。`prefers-reduced-motion: reduce` 下模块直接显示，并关闭自动轮播。E2E 用例 `test_home_carousel_loops_without_a_native_scrollbar` 同时覆盖 2560×1440 首屏保持隐藏、触发阈值前仍等待、越过 100px 阈值后播放、较慢的滚动弹入、自动轮播保留及减少动效行为。
...
当前目录页将筛选工作台分为三层：型号搜索、常用快捷条件和按需展开的高级条件。每页卡片数的 12/24/48 选项固定放在型号搜索框右侧，变更后同步到 URL；高级条件按“快速定位、外形尺寸、传感性能、按键微动、材质渠道”分组，使用键盘可操作的原生单选控件和五段绿色滑轨显示当前分类，桌面端位于筛选内容左侧，移动端排列在筛选内容上方；已启用条件在结果区以可移除标签显示。

鼠标卡片的产品图使用固定 4:3 媒体画布和 `object-fit: contain`，完整保留原图比例，避免裁切产品主体；无图时保持同等尺寸的占位画布，确保列表扫描时卡片高度稳定。

同一筛选类别内的多选使用 OR，不同筛选类别之间使用 AND。按评分排序时，样本数不少于 5 的鼠标按评分排序在前；低样本和无评价项目统一排在后方，再按评价数和 ID 稳定排序。

#### 分页

- 目录页默认每页 12 张卡片，可切换为 24 或 48 张；
- 后端分页，禁止一次返回全量记录；
- 页码、总条数和当前筛选条件始终可见；
...
### 5.9 工具页 `/mouse-test` 与 `/sensitivity`

桌面主导航将鼠标测试和灵敏度换算收纳在“工具”菜单中，按钮只显示“工具”文字，不附带展开箭头。下拉项不显示 `M1`、`360` 等装饰代码，两项统一为单列标题与说明；菜单宽度固定为 `188px`，两项保持相同的高度、内边距和 `6px` 选中背景圆角。鼠标悬浮、键盘聚焦或点击按钮时展开；按钮和浮层之间的视觉间距由与菜单同宽的透明悬浮桥覆盖，指针从按钮移动到列表的途中不会关闭。指针进入下拉列表后移出列表时自动关闭；点击菜单外部、点击选项、按下 Esc 或键盘焦点离开时同样关闭。移动端底部导航保持四个主入口，点击“工具”后在导航上方展开同样的两个页面入口。

鼠标测试页使用原创的 GPW 风格对称俯视 SVG 轮廓，提供左键、右键、滚轮键、前侧键、后侧键五个可聚焦区域。点击轮廓或使用 Enter/Space 后，对应区域高亮并更新按键名称、累计次数、完成进度和最近输入。组件挂载期间，页眉底部到页脚顶部之间的整个视口宽度都是实体输入捕获范围，包括内容壳左右的空白；页眉与页脚自身不触发。浏览器 `PointerEvent.button` 的 `0/1/2/3/4` 映射真实五键输入，`WheelEvent.deltaY` 分别记录滚轮上滚与下滚，因此进度共包含七项输入。上滚与下滚都会点亮 SVG 滚轮本体，并分别显示上/下方向箭头、让滚轮纹路向对应方向短距位移；滚轮中键只使用按压高亮，不显示方向箭头。方向反馈只使用 `transform` 与 `opacity`，在 `prefers-reduced-motion` 下取消位移但保留方向高亮。捕获范围内屏蔽右键菜单和辅助点击默认行为，滚轮仍保留页面的正常滚动。重置操作清空全部计数和历史记录。该路由隐藏公共社媒/反馈浮层和底部对比托盘，避免干扰测试。

浏览器断言位于 `frontend/e2e/test_smoke.py` 的 `test_tools_menu_opens_the_mouse_input_tester`，覆盖桌面菜单宽度、下拉项无装饰代码且几何一致、从按钮慢速移入、移出列表自动关闭、选项点击、SVG 五键反馈、内容壳外实体右键捕获、页眉排除、滚轮双向计数及 SVG 方向反馈、重置以及移动端工具菜单边界。
...
公共页面根节点使用纵向 flex 布局，页眉、路由主内容和页脚依次排列。页眉与页脚必须使用 `flex: 0 0 auto`，禁止拉伸或压缩；短页面只允许中间的路由 `<main>` 使用 `flex: 1 0 auto` 吸收剩余高度，长页面继续按内容自然增长。除首页需要为“近期新品”保留稳定滚动距离外，目录、对比、推荐、工具、登录、账户和协议等页面不得再用 `100dvh` 或 `calc(100dvh - 页头高度)` 强制撑高自身。桌面 2560×1440 下，公共页眉保持约 68px 并贴顶，页脚保持约 161px 并贴底，内容不得与页脚重叠。桌面页脚按品牌、版权和导航的实际内容宽度分配网格列，导航间距随可用宽度收紧，完整中文标签不得拆行；窄屏继续沿用移动端双列导航。E2E 用例 `test_short_public_pages_keep_header_and_footer_geometry_stable_on_2k` 和 `test_footer_navigation_labels_stay_on_one_line` 分别验证几何与文本排版契约。

桌面广告栏使用 CSS 视口单位定位，但广告栏显示时，内容壳必须同步预留左右广告、边缘和间距占用的空间。预留计算与广告开关解耦，因此运营配置切换不会让主内容水平跳动。

广告栏仅在至少 `90rem` 宽且 `38.75rem` 高的 CSS 视口显示。左右广告列固定为 `220px`，内容壳按 CSS 视口扣除广告列、边缘和间距后收缩，广告保持在独立侧栏，不覆盖内容。此约束适用于页头、页脚、所有公开页面的 `section-shell`、主页面壳和公开账户壳，页面级样式不得用固定宽度绕过它。Windows 150% 缩放下，2560 物理像素屏幕通常对应约 1707 CSS px；该视口必须逐路由验证广告栏位于内容壳外侧，且页面没有横向溢出。布局测试位于 `frontend/e2e/test_smoke.py` 的宽屏响应式和广告栏测试中。

### 11.4 全局导航与社媒入口

公共顶部导航使用约 68% 不透明度的深色背景和 `18px` 背景模糊，移动端底部导航使用约 72% 不透明度并沿用同一模糊强度；两者必须保留分隔线与足够的文字对比度，不得退回接近实色的背景。桌面主导航保持 `40px` 控件高度、正常中文字符间距和紧凑内边距。当前页面使用低透明度绿色径向光与 `15px` 细网格共同表达选中状态，悬浮时网格收紧至 `11px`；不得直接套用大字号、大字距或全局 `.button` 选择器。“工具”是包含鼠标测试与灵敏度换算的菜单按钮，两条工具路由均保持该按钮的选中状态。移动端继续使用简化的纯色选中态，避免在狭窄导航中制造纹理噪声，并通过点击工具按钮展开触摸菜单。浏览器断言 `test_public_navigation_uses_a_translucent_backdrop` 覆盖两套导航的透明度区间与背景模糊。

所有公共页面右下角使用统一浮动操作组展示 Bilibili 社媒入口和反馈按钮，社媒入口位于反馈按钮左侧，避免遮挡目录页左侧首列卡片。链接指向 `https://space.bilibili.com/509985180?spm_id_from=333.1007.0.0` 并以新标签页打开。入口使用五层错位方框动效，鼠标悬浮或键盘聚焦时向左上展开包含平台名和 UID 的资料卡；动效颜色跟随 GearDB 绿色强调色，并遵循 `prefers-reduced-motion`。移动端反馈按钮隐藏，社媒入口单独位于底部导航上方。

社媒入口不在管理后台和开发代码地图中显示。浏览器断言位于 `frontend/e2e/test_smoke.py` 的 `test_public_social_link_opens_bilibili_profile_without_covering_mobile_navigation`。
...
9. 推荐页使用键盘可操作的原生单选控件选择握姿，选中项显示绿色文字与对应段位的绿色滑轨背光，随后可继续标记支撑位置并提交推荐。

推荐流程的浏览器断言位于 `frontend/e2e/test_smoke.py` 的 `test_recommendations_show_exact_and_explained_near_matches`，运行 `cd frontend && npm run test:e2e` 可验证握姿单选、绿色滑轨、支撑涂抹和匹配结果。
