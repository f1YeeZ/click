...
当前仓库尚未包含业务代码，因此本文同时给出推荐技术栈和目录结构。技术栈若已有外部约束，可替换实现方式，但产品范围、数据口径、接口契约和验收标准应保持一致。
...
| --- | --- | --- |
| 鼠标库、详情、筛选、2～4 款对比、固定评价、邮箱登录、基础管理后台、CSV 导入 | 轮廓/3D、自由文本、商品价格、推荐、其他外设品类、逐条公开评价、产品图片 | 邮箱验证、忘记密码、第三方登录、热门模块、收藏、轮廓和推荐 |
...
| 角色 | 能力 |
| --- | --- |
| 访客 | 浏览、搜索、筛选、查看详情、选择并分享对比、查看评价汇总 |
| 注册用户 | 访客全部能力；提交、修改和删除自己的固定模板评价 |
| 管理员 | 用户全部能力；新增/编辑/下架鼠标、批量导入数据、查看评价记录、停用异常评价、维护评价标签文案与启用状态 |
...
- 所有评分均为 1～5 的整数，1 表示很差，5 表示很好。
- 综合评分由各评分维度等权平均得出，保留 1 位小数；客户端不得自行提交综合分。
...
| --- | --- | --- |
| 握持方式 | 单选 | 趴握、抓握、指握、混合 |
| 手长 | 单选 | 小于 17 cm、17 cm（含）～19 cm（不含）、19 cm 及以上 |
| 使用时长 | 单选 | 少于 7 天、7～29 天、30～179 天、180 天及以上 |
...
优点标签，最多选择 3 个；括号内为不可变 code：

- 轻量（`lightweight`）、握持舒适（`comfortable`）、按键清脆（`crisp_clicks`）、滚轮清晰（`defined_scroll`）、做工扎实（`solid_build`）、传感稳定（`stable_sensor`）、连接稳定（`stable_connection`）、续航优秀（`great_battery`）、性价比高（`good_value`）。

问题标签，最多选择 3 个；括号内为不可变 code：

- 尺寸不合适（`size_mismatch`）、重心不适（`balance_issue`）、按键偏硬（`clicks_too_stiff`）、按键偏软（`clicks_too_soft`）、滚轮问题（`scroll_issue`）、做工问题（`build_issue`）、连接不稳（`unstable_connection`）、续航较差（`poor_battery`）、价格偏高（`price_high`）。

表单提交前需要确认：评分维度全部完成，上下文选项全部完成；优点和问题标签允许为空。

标签 code 和类别发布后不可修改；管理员只能修改显示文案、排序和启用状态。并列热门标签按 `sort_order`、再按 code 升序；公开汇总每类最多返回 5 个。
...
- 支持退出登录；
- 忘记密码和邮箱验证可在第一轮上线后补充，若不实现需在界面明确说明。
...
- 所有评分字段增加 `CHECK (score BETWEEN 1 AND 5)`；
...
```json
{
  ...
  "dimensionDistributions": {
    "comfort": { "1": 0, "2": 1, "3": 4, "4": 12, "5": 10 }
  },
  "topPros": [{ "code": "lightweight", "label": "轻量", "count": 18 }],
  "topCons": [{ "code": "price_high", "label": "价格偏高", "count": 7 }],
  "lowSample": false
}
```

`dimensionDistributions` 对五个整数评分维度分别统计 1～5 分数量，不对带小数的综合评分做主观分桶。标签被管理员停用后不可再选择，历史关联保留用于审计，但不再进入公开汇总。

单条 `overall_score = (五项整数评分之和) / 5`，结果天然为 0.2 的倍数，存为 `DECIMAL(2,1)`。汇总时先使用未舍入的数据库数值计算：`overallAverage` 为所有有效评价的 `overall_score` 算术平均值，各维度均分为该维度整数分的算术平均值；仅在 API 响应阶段四舍五入保留 1 位。
...
```json
{
  "error": {
    ...
    "message": "提交内容不符合要求",
    "fields": { "comfortScore": "评分必须为 1 到 5" },
    "requestId": "req_xxx"
  }
}
```
...
| 方法 | 路径 | 请求/响应要点 |
| --- | --- | --- |
| POST | `/api/v1/auth/register` | `{email,password}`；成功返回 JWT 和用户摘要 |
...
| 单条综合分 | 5、4、4、5、4 | `4.4` |
| 两条评价汇总 | `[5,4,4,5,4]` 与 `[3,4,2,5,3]` | 综合均分 `3.9`；维度均分为 `4.0/4.0/3.0/5.0/3.5` |
| 分布 | 上述两条评价的舒适度 | `{1:0,2:0,3:1,4:0,5:1}` |
...
| 评分维度 | 舒适、按键、滚轮、做工、性价比 | 可在开发前调整，发布后不宜频繁修改口径 |
| 综合分 | 五维等权平均 | 简单透明，不在 MVP 引入权重模型 |
...
1. 首批鼠标数据的来源、数量和授权方式；
2. 是否已有既定前后端技术栈、部署平台和域名；
3. 固定评价维度、标签文案和低样本阈值是否需要调整；
4. 是否必须在首版提供邮箱验证、忘记密码和第三方登录；
