...
桌面主导航将鼠标测试和灵敏度换算收纳在“工具”菜单中，按钮只显示“工具”文字，不附带展开箭头。鼠标测试下拉说明统一为“按键、回报率与反应”。下拉项不显示 `M1`、`360` 等装饰代码，两项统一为单列标题与说明；菜单宽度固定为 `188px`，两项保持相同的高度、内边距和 `6px` 选中背景圆角。鼠标悬浮、键盘聚焦或点击按钮时展开；按钮和浮层之间的视觉间距由与菜单同宽的透明悬浮桥覆盖，指针从按钮移动到列表的途中不会关闭。指针进入下拉列表后移出列表时自动关闭；点击菜单外部、点击选项、按下 Esc 或键盘焦点离开时同样关闭。移动端底部导航保持四个主入口，点击“工具”后在导航上方展开同样的两个页面入口。

鼠标测试页通过“按键测试 / 回报率测试 / 反应测试”页内标签合并三种诊断，默认进入按键测试。桌面模式切换器复用推荐页握法选择器的纵向视觉语言并放在当前测试内容左侧：每项显示中文标题、简短说明和右侧英文代码，选中项同时使用绿色文字、整行浅色底和左侧发光轨道表达状态；不再显示复古金属面板、LED 或电平柱。宽度不超过 `1100px` 时，切换器保持纵向列表并移动到测试内容上方。标签继续使用完整的键盘标签页模式：上下或左右方向键在相邻模式间移动，Home/End 跳到首尾；每项触控高度不得小于 44px，发光轨道位移在 `prefers-reduced-motion` 下缩短为近乎即时。按键模式使用原创的 GPW 风格对称俯视 SVG 轮廓，提供左键、右键、滚轮键、前侧键、后侧键五个可聚焦区域。点击轮廓或使用 Enter/Space 后，对应区域高亮并更新按键名称、累计次数、完成进度和最近输入。按键模式启用期间，页眉底部到页脚顶部之间的整个视口宽度都是实体输入捕获范围，包括内容壳左右的空白；页眉与页脚自身不触发。浏览器 `PointerEvent.button` 的 `0/1/2/3/4` 映射真实五键输入，`WheelEvent.deltaY` 分别记录滚轮上滚与下滚，因此进度共包含七项输入。上滚与下滚都会点亮 SVG 滚轮本体，并分别显示上/下方向箭头、让滚轮纹路向对应方向短距位移；滚轮中键只使用按压高亮，不显示方向箭头。方向反馈只使用 `transform` 与 `opacity`，在 `prefers-reduced-motion` 下取消位移但保留方向高亮。捕获范围内屏蔽右键菜单和辅助点击默认行为，滚轮仍保留页面的正常滚动。重置操作清空全部计数和历史记录。

回报率模式只在用户点击“开始测试”后采集鼠标 `pointermove`，采样范围为页眉底部到页脚顶部之间的整个视口宽度，包括内容壳左右的页面空白；页眉和页脚自身不计数。界面中的目标区仅承担采样状态反馈，不再限制输入范围。事件处理优先展开 `PointerEvent.getCoalescedEvents()` 返回的合并事件。统计使用最近一秒样本计算当前 Hz，并以连续移动期间不超过 100ms 的有效相邻事件间隔计算测试平均 Hz，避免暂停或静止时间拉低结果；同时记录测试期间峰值、样本数、时长、最近趋势和最接近的 125/250/500/1000/2000/4000/8000 Hz 标准档位。用户可暂停、继续或重测；暂停以及切回按键模式后不得继续计数，重测清空全部统计。页面必须显示浏览器事件合并、系统调度和屏幕刷新率会影响估算结果的限制说明。为了限制长时间测试的内存占用，当前速率窗口最多保留最近 30,000 个时间戳，总样本数仍独立累计。该路由继续隐藏公共社媒/反馈浮层和底部对比托盘，避免干扰测试。

反应模式以五轮视觉点击测试统计人体反应时间。每轮点击开始后随机等待 1.2–3 秒，等待阶段不得显示倒计时；定时器结束后的下一次 `requestAnimationFrame` 将整个信号区切换为高对比绿色，中央目标同步放大、发光并显示“点击”，底部文字同时显示“现在点击”。提示变化必须同时使用面积、亮度与文本，不能只依赖细边框或颜色。以该帧时间戳到鼠标点击或键盘 Enter/Space 的 `performance.now()` 毫秒差作为成绩。信号前点击必须显示“抢跑了”，本轮不写入成绩；有效结果显示最近一次、平均、最佳和五轮明细。单轮出成绩后，“下一轮”主按钮必须位于信号区几何中心，重置成绩仍保留在区外作为次要操作。用户可取消等待中的本轮、重做抢跑轮、进入下一轮、完成后再测或随时重置成绩；等待中切换到其他测试必须清理定时器与待执行动画帧，并保留已经完成的成绩。页面必须注明结果混合了人体反应、鼠标、系统、浏览器和显示器延迟，只适用于相同设备与环境下的相对比较，不得表述为鼠标硬件延迟。

浏览器断言位于 `frontend/e2e/test_smoke.py` 的 `test_tools_menu_opens_the_mouse_input_tester`，覆盖桌面菜单宽度、下拉项无装饰代码且几何一致、从按钮慢速移入、移出列表自动关闭、选项点击、模式列表位于测试内容左侧、三项纵向几何、英文代码、选中滑轨、上下键与 Home/End 键盘导航、窄屏列表位于内容上方、SVG 五键反馈、内容壳外实体右键捕获、页眉排除、滚轮双向计数及 SVG 方向反馈、按键重置、回报率全页面宽度采样与页眉排除、暂停不再计数、继续、重测归零、反应测试抢跑不计分、强化信号、有效键盘触发、“下一轮”居中、切换模式取消等待、五轮完成态、375px 放大字号与减少动效、移动端触控尺寸以及工具菜单边界。回报率纯计算边界由 `frontend/src/utils/pollingRate.test.js` 覆盖；运行 `cd frontend && npm test -- --run` 和 `npm run test:e2e` 可分别验证统计逻辑与完整浏览器流程。
...
广告位功能已删除。公开配置 `/api/v1/config` 不再返回广告总开关或左右广告字段，后台系统运营页不再提供广告图片、链接和开关设置，前台也不创建广告节点。数据库迁移 `V11__remove_advertising_settings.sql` 会清理历史 `advertising.*` 设置；服务端继续收到旧广告设置更新请求时必须返回“不支持的系统设置”。浏览器测试会让模拟的旧版公开配置和后台设置响应继续携带广告字段，断言页面仍无 `[data-ad-slot]` 节点，并确认旧广告设置既不会显示专用卡片，也不会混入通用设置列表。

广告功能删除后仍保留原有的宽屏内容占比和居中位置，左右空白作为纯版式留白存在，不创建广告节点，也不恢复广告配置。在宽度至少 `1440px`、高度至少 `620px` 的 CSS 视口中，公共内容壳使用与原布局等价的两侧预留公式，并以 `1440px` 为最大宽度；这样 Windows 150% 缩放等场景下目录卡片不会因广告删除而整体放大或横向移位。页面仍不得产生横向溢出，宽屏目录的最大宽度与缩放后留白分别由 `test_wide_catalog_preserves_the_fixed_shell_with_readable_type` 和 `test_wide_catalog_keeps_original_side_gutters_without_ads` 覆盖，后者同时确认页面不存在 `[data-ad-slot]` 节点。
